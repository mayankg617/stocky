              ...
              <div class="warehouse-filter">
                ...
                >
                  ...
                  <template v-slot:selected-option="option">
                    <i class="i-Home1 mr-2"></i>
                    {{ option ? (warehouses.find(w => w.id === option) ? warehouses.find(w => w.id === option).name : '') : $t('Filter_by_warehouse') }}
                  </template>
                </v-select>
              </div>
      ...
      <div class="row mb-4">
        <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
          <div class="stat-card expiring-card">
            ...
            <div class="stat-card-content">
              <p class="stat-card-label">{{ $t('Expiring_in_30_days') || 'Expiring in 30 days' }}</p>
              <h3 class="stat-card-value">
                <span class="badge badge-warning">{{ expiring_batches_count }}</span>
              </h3>
              <div class="mt-2 small">
                <div v-if="expiring_batches.length">
                  <div v-for="(b, idx) in expiring_batches.slice(0,5)" :key="b.id" class="d-flex justify-content-between mb-1">
                    <div>
                      <strong>{{ b.product_name }}</strong>
                      <div class="small text-muted">Batch: {{ b.batch_no || '--' }}</div>
                    </div>
                    <div class="text-right">
                      <div class="small text-muted">{{ b.expiry_date }}</div>
                      <div class="small">Qty: {{ b.quantity }}</div>
                    </div>
                  </div>
                </div>
                <div v-else class="text-muted">{{ $t('No_data') }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
          <div class="stat-card expired-card">
            ...
            <div class="stat-card-content">
              <p class="stat-card-label">{{ $t('Expired_Batches') || 'Expired' }}</p>
              <h3 class="stat-card-value">
                <span class="badge badge-danger">{{ expired_batches_count }}</span>
              </h3>
              <div class="mt-2 small">
                <div v-if="expired_batches.length">
                  <div v-for="(b, idx) in expired_batches.slice(0,5)" :key="b.id" class="d-flex justify-content-between mb-1">
                    <div>
                      <strong>{{ b.product_name }}</strong>
                      <div class="small text-muted">Batch: {{ b.batch_no || '--' }}</div>
                    </div>
        ...
        <div class="col-lg-4 col-md-12 col-sm-12 mb-3">
          <div class="info-card">
            <div class="info-card-header">
              <h4 class="info-card-title">{{ $t('Near_Expiry_Alerts') || 'Near expiry alerts' }}</h4>
            </div>
            <div class="info-card-body">
              <div v-if="expiring_batches.length || expired_batches.length">
                <div v-for="b in (expiring_batches.concat(expired_batches)).slice(0,8)" :key="b.id" class="info-card-item small py-2">
                  <div class="d-flex justify-content-between align-items-center">
                    <div>
                      <div><strong>{{ b.product_name }}</strong></div>
                      <div class="small text-muted">Batch: {{ b.batch_no || '--' }} • {{ b.warehouse_id ? (warehouses.find(w => w.id === b.warehouse_id) ? warehouses.find(w => w.id === b.warehouse_id).name : '') : '' }}</div>
                    </div>
                    <div class="text-right">
                      <div v-if="new Date(b.expiry_date) <= new Date()" class="badge badge-danger">{{ $t('Expired') || 'Expired' }}</div>
                      <div v-else class="badge badge-warning">{{ $t('Near_Expiry') || 'Near Expiry' }}</div>
                      <div class="small">{{ b.expiry_date }}</div>
                    </div>
                  </div>
                </div>
              </div>
    ...
    all_dashboard_data() {
      ...
      axios
        ...
        .then(response => {
          ...
          // Batch expiry widgets data
          this.expired_batches_count = Number(responseData.expired_batches_count) || 0;
          this.expiring_batches_count = Number(responseData.expiring_batches_count) || 0;
          this.expired_batches = Array.isArray(responseData.expired_batches) ? responseData.expired_batches.slice(0, 10) : [];
          this.expiring_batches = Array.isArray(responseData.expiring_batches) ? responseData.expiring_batches.slice(0, 10) : [];
...
.stat-card.expiring-card .stat-card-content .badge,
.stat-card.expired-card .stat-card-content .badge {
  font-size: 1.25rem;
