    ...
    <b-card v-show="!pageLoading" class="mb-3 p-0 overflow-hidden soft-shadow">
      <div class="header-hero d-flex align-items-center p-3">
        ...
        <div class="flex-grow-1">
          <h4 class="mb-1 text-white font-weight-bold client-name">{{ client.name || '-' }}</h4>
          <div class="client-details">
            <span class="mr-3">{{ $t('Code') }}: <b>{{ client.code || '-' }}</b></span>
            <span class="mr-3">{{ $t('Village') }}: <b>{{ client.city || '-' }}</b></span>
            <span class="mr-3">{{ $t('Country') }}: <b>{{ client.country || '-' }}</b></span>
            <span class="mr-3">{{ $t('Aadhar_Number') }}: <b>{{ client.tax_number || '-' }}</b></span>
          </div>
        </div>
...
.header-hero { background: linear-gradient(135deg, #3b5bdb 0%, #5a67d8 100%); color: #fff; }
.header-hero h4 {
