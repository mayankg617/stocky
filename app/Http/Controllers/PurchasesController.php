                    ...
                    if ($value['product_variant_id'] !== null) {
                        ...
                        if ($unit && $product_warehouse) {
                            if ($unit->operator == '/') {
                                ...
                                $product_warehouse->qte += $addedQty;
                            } else {
                                $addedQty = $value['quantity'] * $unit->operator_value;
                                $product_warehouse->qte += $addedQty;
                            }
            ...
            if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
                        ...
                        if ($current_Purchase->statut == 'received') {
                            ...
                            if ($value['product_variant_id'] !== null) {
                                ...
                                if ($unit && $product_warehouse) {
                                        ...
                                        // Reduce batch quantity for this purchase detail if exists
                ...
                foreach ($new_purchase_details as $key => $prod_detail) {
                    ...
                    if ($prod_detail['no_unit'] !== 0) {
                            ...
                            if ($prod_detail['product_variant_id'] !== null) {
                                ...
                                if ($unit_prod && $product_warehouse) {
                                        if ($unit_prod->operator == '/') {
                                            $addedQty = $prod_detail['quantity'] / $unit_prod->operator_value;
                                            $product_warehouse->qte += $addedQty;
                                        } else {
                                            $addedQty = $prod_detail['quantity'] * $unit_prod->operator_value;
                                            $product_warehouse->qte += $addedQty;
                                        ...
                                        // Upsert batch record for updated/added detail
                                        if (! empty($prod_detail['batch_no']) || ! empty($prod_detail['expiry_date'])) {
                                            $existing = \DB::table('product_batches')
                                                ->where('purchase_id', $id)
                                                ->where('product_id', $prod_detail['product_id'])
                                                ->where('batch_no', $prod_detail['batch_no'])
                                                ->first();

                                            if ($existing) {
                                                \DB::table('product_batches')
                                                    ->where('id', $existing->id)
                                                    ->update(['quantity' => \DB::raw('quantity + ' . (float) $addedQty), 'updated_at' => Carbon::now()]);
                                            } else {
                                                \DB::table('product_batches')->insert([
                                                    'product_id' => $prod_detail['product_id'],
                                                    'purchase_id' => $id,
                                                    'batch_no' => isset($prod_detail['batch_no']) ? $prod_detail['batch_no'] : null,
                                                    'expiry_date' => isset($prod_detail['expiry_date']) ? $prod_detail['expiry_date'] : null,
                                                    'quantity' => $addedQty,
                                                    'created_at' => Carbon::now(),
                                                    'updated_at' => Carbon::now(),
                                                ]);
                        ...
                        $orderDetails['batch_no'] = isset($prod_detail['batch_no']) ? $prod_detail['batch_no'] : null;
                        $orderDetails['expiry_date'] = isset($prod_detail['expiry_date']) ? $prod_detail['expiry_date'] : null;
                    ...
                    if ($current_Purchase->statut == 'received') {
                        ...
                        if ($value['product_variant_id'] !== null) {
                            ...
                            if ($unit && $product_warehouse) {
                                ...
                                // Reduce batch quantity if batch info exists on detail
                                if (! empty($value['batch_no']) || ! empty($value['expiry_date'])) {
                                    \DB::table('product_batches')
                                        ->where('purchase_id', $id)
                                        ->where('product_id', $value['product_id'])
