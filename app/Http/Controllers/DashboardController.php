...
class DashboardController extends Controller
{
    ...
    public function dashboard_data(Request $request)
    {
        ...
        // Batch expiry widgets: expired, expiring in 30 days, and near-expiry list
        $today = Carbon::today();
        $threshold = Carbon::today()->addDays(30);

        $baseBatchesQuery = DB::table('product_batches')
        ...
        $expired_batches = (clone $baseBatchesQuery)
            ->whereNotNull('product_batches.expiry_date')
            ->whereDate('product_batches.expiry_date', '<=', $today->toDateString())
            ->select('product_batches.id', 'product_batches.batch_no', 'product_batches.expiry_date', 'product_batches.quantity', 'products.name as product_name', 'purchases.warehouse_id')
            ->orderBy('product_batches.expiry_date', 'asc')
            ->get();

        $expiring_batches = (clone $baseBatchesQuery)
            ->whereNotNull('product_batches.expiry_date')
            ->whereDate('product_batches.expiry_date', '>', $today->toDateString())
            ->whereDate('product_batches.expiry_date', '<=', $threshold->toDateString())
            ->select('product_batches.id', 'product_batches.batch_no', 'product_batches.expiry_date', 'product_batches.quantity', 'products.name as product_name', 'purchases.warehouse_id')
            ->orderBy('product_batches.expiry_date', 'asc')
            ->get();
        ...
        return response()->json([
            ...
            'expired_batches_count' => $expired_count ?? 0,
            'expiring_batches_count' => $expiring_count ?? 0,
