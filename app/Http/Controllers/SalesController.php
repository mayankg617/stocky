...
class SalesController extends BaseController
{
        ...
        $sale = \DB::transaction(function () use ($request) {
            ...
            foreach ($data as $key => $value) {
                ...
                $orderDetails[] = [
                    ...
                    'batch_id' => isset($value['batch_id']) ? $value['batch_id'] : null,
                    ...
                    if ($value['product_variant_id'] !== null) {
                        ...
                        if ($unit && $product_warehouse) {
                            if ($unit->operator == '/') {
                                ...
                                $product_warehouse->qte -= $removedQty;
                            } else {
                                $removedQty = $value['quantity'] * $unit->operator_value;
                                $product_warehouse->qte -= $removedQty;
                            }

                            // If there are existing batches for this product in this warehouse, require batch selection
                            $hasBatches = \DB::table('product_batches')
                                ->where('product_id', $value['product_id'])
                                ->where('warehouse_id', $order->warehouse_id)
                                ->where('quantity', '>', 0)
                                ->exists();

                            if ($hasBatches && empty($value['batch_id'])) {
                                throw new \Exception('Batch selection required for product id ' . $value['product_id']);
                            }

                            $product_warehouse->save();

                            // If sale consumed a specific batch, decrement product_batches accordingly
                            if (isset($value['batch_id']) && $value['batch_id']) {
                                $batch = \DB::table('product_batches')->where('id', $value['batch_id'])->first();
                                if ($batch) {
                                    if ($batch->quantity < $removedQty) {
                                        throw new \Exception('Cannot sell more than available in selected batch.');
                                    }
                                    \DB::table('product_batches')->where('id', $value['batch_id'])
                        ...
                        if ($value['product_variant_id'] !== null) {
                            ...
                            if ($product_warehouse && $old_unit) {
                                if ($old_unit->operator == '/') {
                                    $addQty = $value['quantity'] / $old_unit->operator_value;
                                    $product_warehouse->qte += $addQty;
                                } else {
                                    $addQty = $value['quantity'] * $old_unit->operator_value;
                                    $product_warehouse->qte += $addQty;
                                }
                            ...
                            if ($prod_detail['product_variant_id'] !== null) {
                                ...
                                if ($product_warehouse && $unit_prod) {
                                    if ($unit_prod->operator == '/') {
                                        $removedQty = $prod_detail['quantity'] / $unit_prod->operator_value;
                                        $product_warehouse->qte -= $removedQty;
                                    } else {
                                        $removedQty = $prod_detail['quantity'] * $unit_prod->operator_value;
                                        $product_warehouse->qte -= $removedQty;
                                    ...
                                    if ($hasBatches && empty($prod_detail['batch_id'])) {
                                        throw new \Exception('Batch selection required for product id ' . $prod_detail['product_id']);
                                    }

                                    $product_warehouse->save();

                                    if (isset($prod_detail['batch_id']) && $prod_detail['batch_id']) {
                                        $batch = \DB::table('product_batches')->where('id', $prod_detail['batch_id'])->first();
                                        if ($batch) {
                                            if ($batch->quantity < $removedQty) {
                                                throw new \Exception('Cannot sell more than available in selected batch.');
                                            }
                                            \DB::table('product_batches')->where('id', $prod_detail['batch_id'])
                                                ->update(['quantity' => \DB::raw('GREATEST(quantity - ' . (float) $removedQty . ', 0)')]);
                                        }
                                    }
                                }
    ...
    public function create(Request $request)
    {
        ...
        $point_to_amount_rate = $settings && isset($settings->point_to_amount_rate) ? $settings->point_to_amount_rate : 0;
        ...
        return response()->json([
            ...
            'accounts' => $accounts,
            'payment_methods' => $payment_methods,
            'point_to_amount_rate' => $point_to_amount_rate,
        ]);
