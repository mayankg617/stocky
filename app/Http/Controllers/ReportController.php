    ...
    public function stock_Report(request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product->type != 'is_service') {
                ...
                // fetch batches for this product (per selected warehouses)
                $batches_query = \DB::table('product_batches')
                    ->select('id', 'batch_no', 'expiry_date', 'quantity', 'warehouse_id')
                    ->where('product_id', $product->id)
                    ->where('quantity', '>', 0)
                    ->whereIn('warehouse_id', $warehouses_id);

                if ($request->filled('warehouse_id')) {
                    $batches_query->where('warehouse_id', $request->warehouse_id);
                }
                ...
                // compute expired / expiring counts
                $today = Carbon::now()->startOfDay();
                $expiring_threshold = Carbon::now()->addDays(30)->endOfDay();

                $expired_count = collect($batches)->filter(function ($b) use ($today) {
                    return $b->expiry_date && Carbon::parse($b->expiry_date)->lt($today);
                })->count();

                $expiring_count = collect($batches)->filter(function ($b) use ($today, $expiring_threshold) {
                    if (! $b->expiry_date) return false;
                    $d = Carbon::parse($b->expiry_date);
                    return $d->gte($today) && $d->lte($expiring_threshold);
                })->count();

                $item['expired_batches_count'] = $expired_count;
                $item['expiring_batches_count'] = $expiring_count;
